...
class _Parser:
    ...
    def __init__(
        ...
        self._error_parsers = _error_deserializers + (
            extra_error_parsers if extra_error_parsers is not None else []
        )
    ...
    def get_api_error(self, response: requests.Response) -> Optional[DatabricksError]:
        ...
        if not response.ok:
            ...
            return _error_mapper(
                response, {"message": "unable to parse response. " + _unknown_error(response)}
            )
