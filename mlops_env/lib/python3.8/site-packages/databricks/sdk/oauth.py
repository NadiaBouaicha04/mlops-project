...
class Token:
    ...
    def jwt_claims(self) -> Dict[str, str]:
        ...
        try:
            ...
            if len(jwt_split) != 3:
                logger.debug(
                    f"Tried to decode access token as JWT, but failed: {len(jwt_split)} components"
                )
                return {}
...
def retrieve_token(
    client_id, client_secret, token_url, params, use_params=False, use_header=False, headers=None
) -> Token:
...
def get_account_endpoints(
    host: str, account_id: str, client: _BaseClient = _BaseClient()
) -> OidcEndpoints:
    """
...
class TokenCache:
    ...
    def filename(self) -> str:
        ...
        return os.path.expanduser(
            os.path.join(self.__class__.BASE_PATH, hash.hexdigest() + ".json")
        )
