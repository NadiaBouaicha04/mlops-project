...
class DbfsExt(files.DbfsAPI):
    ...
    def open(
        self, path: str, *, read: bool = False, write: bool = False, overwrite: bool = False
    ) -> BinaryIO:
    ...
    def move_(self, src: str, dst: str, *, recursive=False, overwrite=False):
        ...
        if source.is_dir and not recursive:
            ...
            raise IOError(
                f"moving a directory from {src_type} to {dst_type} requires recursive flag"
            )
        # do cross-fs moving
...
class FilesExt(files.FilesAPI):
    ...
    def _download_raw_stream(
        ...
        if start_byte_offset and not if_unmodified_since_timestamp:
            raise Exception(
                "if_unmodified_since_timestamp is required if start_byte_offset is specified"
            )
        ...
        if not isinstance(result.contents, _StreamingResponse):
            raise Exception(
                "Internal error: response contents is of unexpected type: "
...
class _ResilientResponse(_RawResponse):
    ...
    def iter_content(self, chunk_size=1, decode_unicode=False):
        ...
        iterator = self.underlying_response.iter_content(
...
class _ResilientIterator(Iterator):
    ...
    def _extract_raw_response(download_response: DownloadResponse) -> _RawResponse:
        streaming_response: _StreamingResponse = (
            download_response.contents
        )  # this is an instance of _StreamingResponse
    ...
    def _should_recover(self) -> bool:
        if (
            self._total_recovers_count
            == self._api._config.files_api_client_download_max_total_recovers
        ):
    ...
    def _recover(self) -> bool:
        ...
        try:
            ...
            downloadResponse = self._api._download_raw_stream(
                self._file_path, self._offset, self._file_last_modified
            )
            underlying_response = _ResilientIterator._extract_raw_response(downloadResponse)
