    ...
    def select_node_type(
        ...
        for nt in types:
            ...
            if min_cores is not None and nt.num_cores < min_cores:
                continue
            if (min_gpus is not None and nt.num_gpus < min_gpus) or (
                min_gpus == 0 and nt.num_gpus > 0
            ):
                continue
            if local_disk or local_disk_min_size is not None:
                ...
                local_nvme_disks = (
                    int(instance_type.local_nvme_disks) if instance_type.local_nvme_disks else 0
                )
                if instance_type is None or (local_disks < 1 and local_nvme_disks < 1):
                    continue
                local_disk_size_gb = (
                    instance_type.local_disk_size_gb if instance_type.local_disk_size_gb else 0
                )
                local_nvme_disk_size_gb = (
                    instance_type.local_nvme_disk_size_gb
                    if instance_type.local_nvme_disk_size_gb
                    else 0
                )
        ...
        while time.time() < deadline:
            try:
                ...
                if info.state == state.RUNNING:
                    ...
                    raise RuntimeError(
                        f"Cluster {info.cluster_name} is {info.state}: {info.state_message}"
                    )
