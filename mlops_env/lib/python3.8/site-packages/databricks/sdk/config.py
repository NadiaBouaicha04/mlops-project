...
class Config:
    ...
    client_secret: str = ConfigAttribute(
        env="DATABRICKS_CLIENT_SECRET", auth="oauth", sensitive=True
    )
    profile: str = ConfigAttribute(env="DATABRICKS_CONFIG_PROFILE")
    config_file: str = ConfigAttribute(env="DATABRICKS_CONFIG_FILE")
    google_service_account: str = ConfigAttribute(
        env="DATABRICKS_GOOGLE_SERVICE_ACCOUNT", auth="google"
    )
    google_credentials: str = ConfigAttribute(
        env="GOOGLE_CREDENTIALS", auth="google", sensitive=True
    )
    azure_workspace_resource_id: str = ConfigAttribute(
        env="DATABRICKS_AZURE_RESOURCE_ID", auth="azure"
    )
    azure_use_msi: bool = ConfigAttribute(env="ARM_USE_MSI", auth="azure")
    azure_client_secret: str = ConfigAttribute(
        env="ARM_CLIENT_SECRET", auth="azure", sensitive=True
    )
    ...
    metadata_service_url = ConfigAttribute(
        env="DATABRICKS_METADATA_SERVICE_URL", auth="metadata-service", sensitive=True
    )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._credentials_strategy = next(
            s
            for s in [credentials_strategy, credentials_provider, DefaultCredentials()]
            if s is not None
        )
        ...
        if not self.host and self.azure_workspace_resource_id:
            ...
            for environment in ALL_ENVS:
                ...
                if environment.azure_environment.name != azure_env:
                    continue
                if environment.dns_zone.startswith(".dev") or environment.dns_zone.startswith(
                    ".staging"
                ):
                    continue
                return environment
    ...
    def is_account_client(self) -> bool:
        if not self.host:
            return False
        return self.host.startswith("https://accounts.") or self.host.startswith(
            "https://accounts-dod."
        )
    ...
    def user_agent(self):
        ...
        return useragent.to_string(
            self._product_info, [("auth", self.auth_type)] + self._user_agent_other_info
        )
    ...
    def _known_file_config_loader(self):
        if not self.profile and (
            self.is_any_auth_configured or self.host or self.azure_workspace_resource_id
        ):
