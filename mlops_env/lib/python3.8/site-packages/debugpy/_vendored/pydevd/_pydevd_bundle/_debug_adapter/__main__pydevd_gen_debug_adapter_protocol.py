...
def collect_bases(curr_class, classes_to_generate, memo=None):
    ...
    for base_definition in base_definitions:
        if base_definition not in memo:
            ...
            ret.extend(
                collect_bases(classes_to_generate[base_definition], classes_to_generate, memo)
            )
...
def update_class_to_generate_to_json(class_to_generate):
    ...
    for prop_name, prop in prop_name_and_prop:
        ...
        if prop.get("type") == "array":
            to_dict_body.append(
                '    if %(prop_name)s and hasattr(%(prop_name)s[0], "to_dict"):' % namespace
            )
            to_dict_body.append(
                "        %(prop_name)s = [x.to_dict() for x in %(prop_name)s]" % namespace
            )
            ...
            if prop["type"].__class__ == Ref:
                ...
                if prop["type"] == "array":
                    ...
                    if ref is not None:
                        ...
                        init_body.append(
                            "        for o in self.%(prop_name)s:" % dict(prop_name=prop_name)
                        )
                        init_body.append(
