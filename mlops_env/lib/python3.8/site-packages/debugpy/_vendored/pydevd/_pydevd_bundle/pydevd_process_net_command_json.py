    ...
    if not isinstance(rules, list):
        ...
        for rule in rules:
            ...
            if path is None and module is None:
                on_error(
                    'Invalid "rule" (expected dict with "path" or "module"). Found: %s' % (rule,)
                )
                continue
...
class PyDevJsonCommandProcessor(object):
            ...
            if DebugInfoHolder.DEBUG_TRACE_LEVEL >= 1:
                pydev_log.info(
                    ...
                    % (
                        ...
                        json.dumps(
                            request.to_dict(update_ids_to_dap=True), indent=4, sort_keys=True
                        ),
                    )
                )
    ...
    def _set_debug_options(self, py_db, args, start_reason):
        ...
        for w in watch_dirs:
            try:
                new_watch_dirs.add(
                    pydevd_file_utils.get_path_with_real_case(pydevd_file_utils.absolute_path(w))
                )
            except Exception:
    ...
    def on_setfunctionbreakpoints_request(self, py_db, request):
        ...
        for bp in arguments.breakpoints:
            ...
            function_breakpoints.append(
                FunctionBreakpoint(
                    bp["name"], condition, expression, suspend_policy, hit_condition, is_logpoint
                )
            )

            # Note: always succeeds.
            breakpoints_set.append(
                pydevd_schema.Breakpoint(verified=True, id=self._next_breakpoint_id()).to_dict()
            )
    ...
    def on_stacktrace_request(self, py_db, request):
        ...
        self.api.request_stack(
            py_db, request.seq, thread_id, fmt=fmt, start_frame=start_frame, levels=levels
        )
    ...
    def on_source_request(self, py_db, request):
        ...
        if source_reference != 0:
            ...
            if content is None:
                ...
                pydev_log.debug(
                    "Found frame id: %s for source reference: %s", frame_id, source_reference
                )
