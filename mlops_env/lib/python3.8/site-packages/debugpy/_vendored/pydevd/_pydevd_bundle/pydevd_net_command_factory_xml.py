...
class NetCommandFactory(object):
    ...
    def make_io_message(self, msg, ctx):
        ...
        try:
            ...
            return NetCommand(
                str(CMD_WRITE_TO_CONSOLE), 0, '<xml><io s="%s" ctx="%s"/></xml>' % (msg, ctx)
            )
        except:
            return self.make_error_message(0, get_exception_traceback_str())
    ...
    def make_thread_resume_single_notification(self, thread_id):
        try:
            return NetCommand(
                CMD_THREAD_RESUME_SINGLE_NOTIFICATION, 0, json.dumps({"thread_id": thread_id})
            )
        except:
            return self.make_error_message(0, get_exception_traceback_str())
    ...
    def make_reloaded_code_message(self, seq, reloaded_ok):
        try:
            return NetCommand(
                CMD_RELOAD_CODE, seq, '<xml><reloaded ok="%s"></reloaded></xml>' % reloaded_ok
            )
        except Exception:
            return self.make_error_message(seq, get_exception_traceback_str())
    ...
    def make_send_curr_exception_trace_message(
        ...
        try:
            ...
            payload = (
                str(curr_frame_id) + "\t" + exc_type + "\t" + exc_desc + "\t" + thread_suspend_str
            )
                ...
                try:
                    ...
                    while frame is not None:
                        if (
                            ...
                            if arg is not None:
                                exc_type, exc_desc, _thread_suspend_str, thread_stack_str = (
