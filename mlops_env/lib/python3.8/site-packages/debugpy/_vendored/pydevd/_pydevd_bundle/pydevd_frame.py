...
def is_unhandled_exception(container_obj, py_db, frame, last_raise_line, raise_lines):
    ...
    if frame.f_lineno in raise_lines:
        ...
        if try_except_infos is None:
            container_obj.try_except_infos = try_except_infos = py_db.collect_try_except_info(
                frame.f_code
            )
    ...
    def handle_user_exception(self, frame):
        ...
        if exc_info:
            return handle_exception(
                self._args[0], self._args[3], frame, exc_info[0], EXCEPTION_TYPE_USER_UNHANDLED
            )
        return False
        ...
        try:
            ...
            try:
                ...
                if py_db.show_return_values:
                    if is_return and (
                        (
                            info.pydev_step_cmd
                            in (CMD_STEP_OVER, CMD_STEP_OVER_MY_CODE, CMD_SMART_STEP_INTO)
                            and (self._is_same_frame(stop_frame, frame.f_back))
                        )
                        or (
            ...
            try:
                            ...
                            if not filename.endswith(PYDEVD_IPYTHON_CONTEXT[0]):
                                ...
                                while f is not None:
                                    if f.f_code.co_name == PYDEVD_IPYTHON_CONTEXT[1]:
                                        ...
                                        if (
                                            f2 is not None
                                            and f2.f_code.co_name == PYDEVD_IPYTHON_CONTEXT[2]
                                        ):
                ...
                if plugin_stop:
                    ...
                    if is_line:
                        self.set_suspend(
                            thread, step_cmd, original_step_cmd=info.pydev_original_step_cmd
                        )
                        self.do_wait_suspend(thread, frame, event, arg)
                ...
                if exception == SystemExit and py_db.ignore_system_exit_code(value):
                    ...
                    for exc_break, is_user_uncaught in check_excs:
                        ...
                        if py_db.exclude_exception_by_filter(exc_break, trace):
                            ...
                            and not py_db.handle_breakpoint_condition(info, exc_break, frame)
                        ):
                            should_stop = False
                            ...
                            if (
                                ...
                                was_just_raised and py_db.skip_on_exceptions_thrown_in_same_context
                            ):
                ...
                if exc_lineno not in merged:  # Note: check on merged but update lines_ignored.
                    try:
                        line = linecache.getline(
                            absolute_filename, exc_lineno, check_trace_obj.tb_frame.f_globals
                        )
                    except:
                        pydev_log.exception(
