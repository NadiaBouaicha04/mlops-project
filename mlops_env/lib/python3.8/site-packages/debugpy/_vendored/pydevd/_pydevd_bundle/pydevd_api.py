...
class PyDevdAPI(object):
    ...
    def request_set_next(
        ...
        if t:
            if original_filename is not None:
                ...
                pydev_log.debug(
                    "Set next (after path translation) in: %s line: %s", translated_filename, line
                )
                ...
                if multi_mapping_applied:
                    pydev_log.debug(
                        "Set next (after source mapping) in: %s line: %s", translated_filename, line
                    )
                    line = new_line
    ...
    def filename_to_str(self, filename):
        ...
        if filename.__class__ != str:
            raise AssertionError(
                "Expected to have str on Python 3. Found: %s (%s)" % (filename, filename.__class__)
            )
        return filename
    ...
    def add_breakpoint(
        ...
        pydev_log.debug(
            "Breakpoint (after path translation) in: %s line: %s", translated_filename, line
        )
        func_name = self.to_str(func_name)
    ...
    def remove_breakpoint(self, py_db, received_filename, breakpoint_type, breakpoint_id):
        ...
        if file_to_id_to_breakpoint is None:
            pydev_log.critical(
                "Error removing breakpoint. Cannot handle breakpoint of type %s", breakpoint_type
            )
    ...
    def set_dont_trace_start_end_patterns(self, py_db, start_patterns, end_patterns):
        ...
        def custom_dont_trace_external_files(abs_path):
            ...
            return normalized_abs_path.startswith(start_patterns) or normalized_abs_path.endswith(
                end_patterns
            )
    ...
    def _terminate_child_processes_linux_and_mac(self, dont_terminate_child_pids):
        ...
        def list_children_and_stop_forking(initial_pid, stop=True):
            ...
            list_popen = self._popen(
                ["pgrep", "-P", str(initial_pid)], stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
        ...
        for _ in range(50):  # Try this at most 50 times before giving up.
            ...
            for pid in children_pids:
                if pid not in previously_found:
                    ...
                    self._call(
                        ["kill", "-KILL", str(pid)], stdout=subprocess.PIPE, stderr=subprocess.PIPE
                    )
