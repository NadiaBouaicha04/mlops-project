...
class NetCommandFactoryJson(NetCommandFactory):
        ...
        try:
            ...
            for (
                ...
                module_events.extend(
                    self.modules_manager.track_module(filename_in_utf8, module_name, frame)
                )
                ...
                if line_col_info is not None:
                    try:
                        ...
                        if DebugInfoHolder.DEBUG_TRACE_LEVEL >= 2:
                            pydev_log.exception(
                                "Unable to get line from linecache for file: %s", original_filename
                            )
                    else:
                        if line_text:
    ...
    def make_thread_suspend_message(
        ...
        pydev_log.debug(
            "Sending per-thread thread suspend notification (stop_reason: %s)", stop_reason
        )
