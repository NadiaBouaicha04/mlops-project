...
def _get_line(op_offset_to_line, op_offset, firstlineno, search=False):
    ...
    raise AssertionError(
        "Unable to find line for offset: %s.Info: %s" % (op_offset_original, op_offset_to_line)
    )
...
_Instruction = namedtuple(
    "_Instruction", "opname, opcode, starts_line, argval, is_jump_target, offset, argrepr"
)
    ...
    def _lookahead(self):
        ...
        if fullrepr is not None and self.instructions:
            ...
            if self.instructions[0].opname == "RETURN_VALUE":
                ...
                return (
                    msg(
                        next_instruction, "return ", line=self.min_line(next_instruction, fullrepr)
                    ),
                    fullrepr,
                )
