...
class PoFileParser:
    ...
    def __init__(
        self, catalog: Catalog, ignore_obsolete: bool = False, abort_invalid: bool = False
    ) -> None:
        self.catalog = catalog
    ...
    def _finish_current_message(self) -> None:
        if self.messages:
            if not self.translations:
                self._invalid_pofile(
                    "", self.offset, f"missing msgstr for msgid '{self.messages[0].denormalize()}'"
                )
    ...
    *,
    ...
    for message in _sort_messages(catalog, sort_by=sort_by):
        ...
        if not no_location:
            ...
            try:
                locations = sorted(
                    message.locations, key=lambda x: (x[0], isinstance(x[1], int) and x[1] or -1)
                )
