...
def test_ipython_prompt():
    for sample, expected in [
        ...
        assert ipt2.ipython_prompt(sample.splitlines(keepends=True)) == expected.splitlines(
            keepends=True
        )
...
def test_leading_empty_lines():
    for sample, expected in [LEADING_EMPTY_LINES, ONLY_EMPTY_LINES]:
        assert ipt2.leading_empty_lines(sample.splitlines(keepends=True)) == expected.splitlines(
            keepends=True
        )
