...
class TestInt:
    ...
    def test_invalid_input(self):
        ...
        assert result.errors[0].message == 'Int cannot represent non-integer value: "20"'
...
class TestFloat:
    ...
    def test_invalid_input(self):
        ...
        assert result.errors
        assert len(result.errors) == 1
        assert result.errors[0].message == 'Float cannot represent non numeric value: "20"'

        result = schema.execute('{ optional { float(input: "a") } }')
        assert result.errors
        assert len(result.errors) == 1
        assert result.errors[0].message == 'Float cannot represent non numeric value: "a"'

        result = schema.execute("{ optional { float(input: true) } }")
        assert result.errors
        assert len(result.errors) == 1
        assert result.errors[0].message == "Float cannot represent non numeric value: true"
...
class TestBoolean:
    ...
    def test_invalid_input(self):
        ...
        assert result.errors
        assert len(result.errors) == 1
        assert result.errors[0].message == 'Boolean cannot represent a non boolean value: "True"'
        ...
        assert result.errors
        assert len(result.errors) == 1
        assert result.errors[0].message == 'Boolean cannot represent a non boolean value: "a"'

        result = schema.execute("{ optional { bool(input: 1) } }")
        assert result.errors
        assert len(result.errors) == 1
        assert result.errors[0].message == "Boolean cannot represent a non boolean value: 1"

        result = schema.execute("{ optional { bool(input: 0) } }")
        assert result.errors
        assert len(result.errors) == 1
        assert result.errors[0].message == "Boolean cannot represent a non boolean value: 0"
...
class TestString:
    ...
    def test_invalid_input(self):
        ...
        assert result.errors
        assert len(result.errors) == 1
        assert result.errors[0].message == "String cannot represent a non string value: 1"

        result = schema.execute("{ optional { string(input: 3.2) } }")
        assert result.errors
        assert len(result.errors) == 1
        assert result.errors[0].message == "String cannot represent a non string value: 3.2"

        result = schema.execute("{ optional { string(input: true) } }")
        assert result.errors
        assert len(result.errors) == 1
        assert result.errors[0].message == "String cannot represent a non string value: true"
