...
def _goes_to_param_name(param_name, context, potential_name):
    ...
    return any(
        param_name.parent_context == p.parent_context and param_name.start_pos == p.start_pos
        for p in found
    )
    ...
    for func_and_argument in arg_callables:
        ...
        for signature in func.get_signatures():
            ...
            for p in process_params(
                list(
                    _remove_given_params(arguments, signature.get_param_names(resolve_stars=False))
                ),
                new_star_count,
            ):
