...
def infer_anonymous_param(func):
    def get_returns(value):
        if value.tree_node.annotation is not None:
            ...
            if any(
                v.name.get_qualified_names(include_module_names=True) == ("typing", "Generator")
                for v in result
            ):
                return ValueSet.from_sets(
                    v.py__getattribute__("__next__").execute_annotation() for v in result
                )
            return result
    ...
    def wrapper(param_name):
        ...
        if is_pytest_param:
            ...
            if fixtures:
                return ValueSet.from_sets(
                    get_returns(value) for fixture in fixtures for value in fixture.infer()
                )
        return func(param_name)
