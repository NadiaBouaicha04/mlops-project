...
def _replace(
    nodes, expression_replacement, extracted, pos, insert_before_leaf=None, remaining_prefix=None
):
...
def _suite_nodes_to_string(nodes, pos):
    ...
    code = (
        part_of_code + n.get_code(include_prefix=False) + "".join(n.get_code() for n in nodes[1:])
    )
    return prefix, code
