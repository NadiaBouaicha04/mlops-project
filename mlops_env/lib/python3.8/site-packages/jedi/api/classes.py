...
class BaseName:
    ...
    def _get_docstring_signature(self):
        return "\n".join(
            signature.to_string() for signature in self._get_signatures(for_docstring=True)
        )
    ...
    def infer(self, *, only_stubs=False, prefer_stubs=False):
        ...
        return [
            self if n == self._name else Name(self._inference_state, n) for n in resulting_names
        ]
