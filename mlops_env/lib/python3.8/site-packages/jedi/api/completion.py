...
def filter_names(
    inference_state, completion_names, stack, like_name, fuzzy, imported_names, cached_name
):
    comp_dct = set()
    ...
    def _complete_python(self, leaf):
        ...
        if not kwargs_only:
            ...
            completion_names += self._complete_keywords(
                allowed_transitions,
                only_values=not (
                    not current_line or current_line[-1] in " \t.;" and current_line[-3:] != "..."
                ),
            )
...
def _complete_getattr(user_context, instance):
    ...
    """
    names = instance.get_function_slot_names("__getattr__") or instance.get_function_slot_names(
        "__getattribute__"
    )
