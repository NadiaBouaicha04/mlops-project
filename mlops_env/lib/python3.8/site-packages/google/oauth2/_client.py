...
def _token_endpoint_request_no_throw(
    ...
    **kwargs,
    ...
    for _ in retries:
        ...
        response_body = (
            response.data.decode("utf-8") if hasattr(response.data, "decode") else response.data
        )
...
def call_iam_generate_id_token_endpoint(
    ...
    response_data = _token_endpoint_request(
        request,
        iam_id_token_endpoint.replace(credentials.DEFAULT_UNIVERSE_DOMAIN, universe_domain).format(
            signer_email
        ),
        body,
        access_token=access_token,
