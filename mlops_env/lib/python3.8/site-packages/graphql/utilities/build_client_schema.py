...
def build_client_schema(
    ...
    def build_union_def(
        ...
        if maybe_possible_types is None:
            raise TypeError(
                "Introspection result missing possibleTypes:" f" {inspect(union_introspection)}."
            )
    ...
    def build_enum_def(enum_introspection: IntrospectionEnumType) -> GraphQLEnumType:
        if enum_introspection.get("enumValues") is None:
            raise TypeError(
                "Introspection result missing enumValues:" f" {inspect(enum_introspection)}."
            )
    ...
    def build_field(field_introspection: IntrospectionField) -> GraphQLField:
        ...
        if args_introspection is None:
            raise TypeError(
                "Introspection result missing field args:" f" {inspect(field_introspection)}."
            )
    ...
    subscription_type = (
        None if subscription_type_ref is None else get_object_type(subscription_type_ref)
    )
