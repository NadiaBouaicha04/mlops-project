...
class GraphQLScalarType(GraphQLNamedType):
    ...
    def __init__(
        ...
        if parse_literal is not None and (
            ...
            raise TypeError(
                f"{name} must provide" " both 'parse_value' and 'parse_literal' as functions."
            )
    ...
    def to_kwargs(self) -> GraphQLScalarTypeKwargs:
        ...
        return GraphQLScalarTypeKwargs(  # type: ignore
            ...
            parse_value=(
                None if self.parse_value is GraphQLScalarType.parse_value else self.parse_value
            ),
            parse_literal=(
                None
...
class GraphQLField:
    ...
    def __init__(
        ...
        if args is None:
            ...
            isinstance(value, GraphQLArgument) or is_input_type(value) for value in args.values()
        ):
        ...
        if resolve is not None and not callable(resolve):
            raise TypeError(
                "Field resolver must be a function if provided, " f" but got: {inspect(resolve)}."
            )
        if description is not None and not is_description(description):
            raise TypeError("The description must be a string.")
...
class GraphQLObjectType(GraphQLNamedType):
    ...
    def fields(self) -> GraphQLFieldMap:
        ...
        if not isinstance(fields, Mapping) or not all(isinstance(key, str) for key in fields):
            raise TypeError(
                f"{self.name} fields must be specified" " as a mapping with field names as keys."
            )
        if not all(
            isinstance(value, GraphQLField) or is_output_type(value) for value in fields.values()
        ):
            raise TypeError(f"{self.name} fields must be GraphQLField or output type objects.")
        return {
            assert_name(name): (
                value if isinstance(value, GraphQLField) else GraphQLField(value)  # type: ignore
            )
    ...
    def __init__(
        ...
        try:  # check for enum
            ...
            if not isinstance(values, Mapping) or not all(isinstance(name, str) for name in values):
                try:
                    ...
                    raise TypeError(
                        f"{name} values must be an Enum or a mapping" " with value names as keys."
                    ) from error
        ...
        values = {
            assert_enum_value_name(key): (
                value if isinstance(value, GraphQLEnumValue) else GraphQLEnumValue(value)
            )
...
class GraphQLInputObjectType(GraphQLNamedType):
    ...
    def fields(self) -> GraphQLInputFieldMap:
        ...
        if not all(
            ...
            raise TypeError(
                f"{self.name} fields must be" " GraphQLInputField or input type objects."
            )
        return {
            assert_name(name): (
                value
...
class GraphQLNonNull(GraphQLWrappingType[GNT], Generic[GNT]):
    ...
    def __init__(self, type_: GNT):
        ...
        if isinstance(type_, GraphQLNonNull):
            raise TypeError(
                "Can only create NonNull of a Nullable GraphQLType but got:" f" {type_}."
            )
