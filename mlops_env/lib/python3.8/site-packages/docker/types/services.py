        ...
        if type == "bind":
            ...
            if any([labels, driver_config, no_copy, tmpfs_size, tmpfs_mode]):
                raise errors.InvalidArgument(
                    "Incompatible options have been provided for the bind " "type mount."
                )
        elif type == "volume":
            volume_opts = {}
            if no_copy:
            ...
            if any([propagation, tmpfs_size, tmpfs_mode]):
                raise errors.InvalidArgument(
                    "Incompatible options have been provided for the volume " "type mount."
                )
        elif type == "tmpfs":
            tmpfs_opts = {}
            if tmpfs_mode:
            ...
            if any([propagation, labels, driver_config, no_copy]):
                raise errors.InvalidArgument(
                    "Incompatible options have been provided for the tmpfs " "type mount."
                )
...
def _convert_generic_resources_dict(generic_resources):
    ...
    if not isinstance(generic_resources, dict):
        raise errors.InvalidArgument(
            "generic_resources must be a dict or a list " f"(found {type(generic_resources)})"
        )
    resources = []
    ...
    def __init__(self, mode, replicas=None, concurrency=None):
        ...
        if mode not in replicated_modes:
            if replicas is not None:
                raise errors.InvalidArgument(
                    'replicas can only be used for "replicated" or' ' "replicated-job" mode'
                )
...
class Privileges(dict):
    ...
    def __init__(
        ...
        if len(credential_spec) > 1:
            raise errors.InvalidArgument(
                "credentialspec_file and credentialspec_registry are mutually" " exclusive"
            )
