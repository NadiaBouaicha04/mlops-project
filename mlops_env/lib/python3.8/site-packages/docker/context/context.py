...
class Context:
    ...
    def __init__(self, name, orchestrator=None, host=None, endpoints=None, tls=False):
        ...
        if not endpoints:
            ...
            default_endpoint = (
                "docker" if (not orchestrator or orchestrator == "swarm") else orchestrator
            )

            self.endpoints = {
                default_endpoint: {"Host": get_context_host(host, tls), "SkipTLSVerify": not tls}
            }
            return
    ...
    def set_endpoint(
        self, name="docker", host=None, tls_cfg=None, skip_tls_verify=False, def_namespace=None
    ):
        self.endpoints[name] = {
